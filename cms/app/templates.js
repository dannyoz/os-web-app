cms.run(["$templateCache", function($templateCache) {  'use strict';
  ...
  $templateCache.put('cms/app/global/directives/cms-editable.html',
    "<div ng-show=\"type == 'text'\"><input ng-model=\"content\"> <button class=close ng-if=delete ng-click=deleteContent();><span>x</span></button></div><div ng-show=\"type == 'list'\"><div class=edit-list ng-repeat=\"item in content track by $index\"><button class=close ng-if=\"currentList != $index\" ng-click=removeListItem($index);><span>x</span></button> <input ng-model=item ng-focus=\"currentList = $index\" ng-keyup=changeListItem($index,item)></div><button ng-click=addListItem(); class=\"rounded-10px green medium\"><span>+</span></button></div>"
  );
  ...
  $templateCache.put('cms/app/views/editor/editor-views/edit-contact.html',
    "<div class=grid-row><p class=label>Heading</p><div class=editable ng-if=json cms-editable=json[currentView].heading></div><p class=label>Subheading</p><div class=editable ng-if=json cms-editable=json[currentView].subheading></div><p class=label>Success Message</p><div class=wysiwyg text-angular=text-angular name=success ng-model=json[currentView].successMessage></div><p class=label>Error Message</p><div class=wysiwyg text-angular=text-angular name=error ng-model=json[currentView].errorMessage></div></div>"
  );
